...
In our previous project, we had utilized the sliding windows method to help identify lane lines. Our task was a bit more complex now, because rather than identifying peaks in histograms of pixels, we would need to identify actual images of cars and decipher them from non-cars. Luckily, we are equipped with a large supply of car and non-care features, and a classifier that has been trained on them!
